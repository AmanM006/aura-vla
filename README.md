...
- **Architecture**: Tri-camera spatial feature backbone + bidirectional GRU language tower + 1D ResNet with FiLM modulation + Cross-Attention over multimodal tokens (`policy/diffusion_policy.py`).
- **Horizon & Sampling**: 16-step trajectory chunking ($H=16$), 16-step DDIM deterministic trajectory generator.
- **Single Denoise Step Latency (OpenVINO CPU)**: **1.41 ms** (p50: 1.29 ms) ([`evidence/diffusion_train_eval.json`](evidence/diffusion_train_eval.json)).
- **Full 16-Step Trajectory Generation**: **22.52 ms** (**44.4 trajectories/second** on CPU) | **76.74 ms** (**13.0 trajectories/second** on iGPU).
- **Validation Loss**: Converged to **0.08957** across 1,683 sliding-window chunks from 51 domain-randomized demonstration episodes.
- **Temporal Ensembling**: Receding query stride $k=2$, exponential discount kernel $w_i = \exp(-0.05 \cdot \Delta t)$, eliminating actuator chatter and joint jerk.

### D. Manipulation & Task Verification Across Random Seeds

| Evaluation Suite | Policy Mode | Seeds Tested | Sub-Goals Achieved | Success Rate | Evidence File |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Randomized Seeds** | **Calibrated Controller** | 10 seeds (0–9) | **50 / 50** | **100.0%** | [`evidence/eval_seeds_scripted.json`](evidence/eval_seeds_scripted.json) |
| **Negative Control** | **None (Home Hold)** | 10 seeds (0–9) | **0 / 25** | **0.0%** | [`evidence/eval_seeds_none.json`](evidence/eval_seeds_none.json) |
...
| **Dynamic Perturbation** | **Anomalib + Closed-Loop Replan** | Seed 3 + Plate Bump | **Full Table Set** | **100.0%** | [`evidence/demo_seed3_recovery.mp4`](evidence/demo_seed3_recovery.mp4) |
...
### C. Live Observability Dashboard
Launch the FastAPI WebSocket server and open [`http://localhost:8000`](http://localhost:8000) in your browser:
```bash
